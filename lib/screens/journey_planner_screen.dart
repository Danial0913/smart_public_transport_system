...
class _JourneyPlannerScreenState extends State<JourneyPlannerScreen> {
  ...
  Future<void> _chooseStop({
    ...
    setState(() {
      controller.text = selectedStop.name;
      _routeResults = [];
    });
  }
  ...
  Future<void> _useGpsForOrigin() async {
    ...
    try {
      ...
      setState(() {
        _originController.text = nearestStop.name;
        _routeResults = [];
      });
  ...
  Future<void> _findRoutes() async {
    ...
    final origin = _repository.findStop(originText);
    final destination = _repository.findStop(destinationText);
    if (origin == null || destination == null) {
      _showMessage('Choose an origin and destination from the supported stops.');
      return;
    }
  ...
  Widget _buildLocationField({
    ...
    return Row(
      children: [
        ...
        IconButton(
          onPressed: onChoose,
          tooltip: 'Choose from supported stops',
...
class JourneyRouteMap extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final mapCentre = LatLng(
      totalLatitude / allStops.length,
      totalLongitude / allStops.length,
    );
